...
// Интерфейс это способ определения соглашений (контрактов) о том,
// как должен выглядеть тот или иной объект испльзуемый в программе.
...
// Это значит, что он может содержать только описанные в интерфейсе "firstInterface" ключи.
...
// Интерфейс сформулирован таким образом,
// что может регулировать объект с одним свойством, название и содержание которого будет в типе "string".
...
// Производится с помозью ключевого слова "implements".
interface Shape {
    color: string;
}
